...
function shouldShowElevator(elevatorId) {
    ...
    // SHABBAT_ACTIVE=false: Hide from mobile app completely
    if (config.SHABBAT_ACTIVE === false) return false;
...
function updateElevatorDisplay(id, stateData) {
    ...
    if(etaLabel) etaLabel.textContent = `${t.etaPrefix} ${getFloorDisplayName(THIS_SCREEN_FLOOR)} ${t.etaSuffix}`;
    ...
    if(floorEl) floorEl.textContent = getFloorDisplayName(stateData.floor) || '--';
...
function populateFloorGrids(selectedFloorValue) {
    ...
    [ui.modalFloorGrid, ui.settingsFloorGrid].forEach(gridContainer => {
        ...
        for (let i = minFloor; i <= maxFloor; i++) {
            ...
            btn.textContent = (alias !== String(i)) ? `${alias} (${i})` : i;
            btn.dataset.value = i;
