...
// --- TIME TRAVEL DEBUG LOGIC START ---
const globalUrlParams = new URLSearchParams(window.location.search);
const offsetHoursParam = globalUrlParams.get('offsetHours');
// חישוב הסטייה במילישניות (אם אין פרמטר, הסטייה היא 0)
const DEBUG_TIME_OFFSET_MS = offsetHoursParam ? parseFloat(offsetHoursParam) * 3600 * 1000 : 0;

if (DEBUG_TIME_OFFSET_MS !== 0) {
    console.warn(`!!! SYSTEM IN TIME TRAVEL MODE: +${offsetHoursParam} HOURS !!!`);
    document.title = `[TIME TRAVEL +${offsetHoursParam}H] ` + document.title;
}
// --- TIME TRAVEL DEBUG LOGIC END ---
...
function updateJerusalemTime() {
    ...
    // CHANGE: Added DEBUG_TIME_OFFSET_MS to simulate future time
    const now = new Date(Date.now() + serverTimeOffset + DEBUG_TIME_OFFSET_MS);
...
async function checkShabbatStatus() {
    ...
    if (settings && settings.GEO_NAME_ID) {
        try {
            // CHANGE: Added DEBUG_TIME_OFFSET_MS to simulate future time for Shabbat check
            const now = new Date(Date.now() + (serverTimeOffset || 0) + DEBUG_TIME_OFFSET_MS);
        ...
        panel.innerHTML = `
            ...
            <div class="panel-side-info">
                ...
                <div class="eta-display">
                    <div class="floor-label" id="etaLabel-${id}">המעלית תגיע לקומה ${THIS_SCREEN_FLOOR} בעוד</div>
...
function calculateAndShowETAFor(elevatorId, elevatorState) {
    ...
    if (breakActive) {
        // CHANGE: Added DEBUG_TIME_OFFSET_MS to simulate future time for breaks
        const now = new Date(Date.now() + (serverTimeOffset || 0) + DEBUG_TIME_OFFSET_MS);
        let inBreak = false;
